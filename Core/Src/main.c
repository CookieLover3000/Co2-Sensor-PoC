...
/* Platform-specific implementation of the LCD send command function. In general
 * this should use polling transfer. */
						 ...
						 size_t param_size)
{
	...
	HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
	/* send command */
	if (HAL_SPI_Transmit(&hspi1, cmd, cmd_size, BUS_SPI1_POLL_TIMEOUT) ==
		HAL_OK) {
		/* DCX high (data) */
		HAL_GPIO_WritePin(LCD_DC_GPIO_Port, LCD_DC_Pin, GPIO_PIN_SET);
		/* for short data blocks we use polling transfer */
		HAL_SPI_Transmit(&hspi1, (uint8_t *)param, (uint16_t)param_size,
...
/* Platform-specific implementation of the LCD send color function. For better
 * performance this should use DMA transfer. In case of a DMA transfer a
 * callback must be installed to notify LVGL about the end of the transfer.
 */
...
void lvglTask(void *argument)
{
	...
	lcd_disp = lv_st7796_create(LCD_H_RES, LCD_V_RES, LV_LCD_FLAG_NONE,
    ...
    printf("Heap before allocs: %d bytes", xPortGetFreeHeapSize());
	uint32_t buf_size =
    ...
    printf("Heap after first alloc: %d bytes", xPortGetFreeHeapSize());
	...
	// 	return;
	// }
	lv_display_set_buffers(lcd_disp, buf1, NULL, buf_size,
